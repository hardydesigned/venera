...
import { a as apiFetch, g as getActiveScopeKey, t as teamScopeStore } from "./client2.js";
...
async function saveProjectWorkspace(payload) {
  const res = await apiFetch("/projects/workspace", {
    method: "PUT",
...
function uid() {
  return Math.random().toString(36).slice(2, 10) + Math.random().toString(36).slice(2, 6) + Date.now().toString(36).slice(-4);
}
...
function normalizeCard(input, index) {
  const card = input ?? {};
  const parentId = typeof card.parentId === "string" && card.parentId.trim().length > 0 ? card.parentId : null;
  return {
...
function normalizeProject(input) {
  ...
  return {
    ...
    folderId: typeof project.folderId === "string" && project.folderId.trim().length > 0 ? project.folderId : null,
...
function parseState(raw) {
  ...
  try {
    ...
    const folders = Array.isArray(parsed.folders) ? parsed.folders.map((folder, index) => normalizeFolder(folder, index)) : [];
...
function persistState(scopeKey, state) {
...
function createProjectStore() {
  ...
  const store = writable(loadState(activeScopeKey));
  let isHydratingFromServer = false;
  let isRemoteSyncReady = false;
  let saveTimer = null;
  let hydrationToken = 0;
  const scheduleRemoteSave = (scopeKey, state) => {
    if (typeof window === "undefined") return;
    if (saveTimer) {
  ...
  const hydrateFromServer = async (scopeKey) => {
    if (typeof window === "undefined") return;
  ...
  return {
      ...
      store.update((state) => ({
        ...
        projects: [
          ...state.projects,
          {
            ...
            color: input.color,
            folderId: input.folderId && state.folders.some((folder) => folder.id === input.folderId) ? input.folderId : null,
            lists: defaultLists()
          }
        ]
      }));
    ...
    updateProject(projectId, input) {
      store.update((state) => ({
        ...
        projects: state.projects.map((project) => {
          ...
          const nextFolderId = input.folderId === void 0 ? project.folderId : input.folderId && state.folders.some((folder) => folder.id === input.folderId) ? input.folderId : null;
          return {
            ...project,
            name: input.name !== void 0 ? input.name.trim() || project.name : project.name,
            description: input.description !== void 0 ? input.description.trim() : project.description,
    ...
    deleteFolder(folderId) {
      store.update((state) => ({
        ...
        projects: state.projects.map(
          (project) => project.folderId === folderId ? { ...project, folderId: null } : project
        )
      }));
    },
      ...
      store.update((state) => ({
        ...
        projects: state.projects.map(
          (project) => project.id === projectId ? {
            ...project,
            folderId: folderId && state.folders.some((folder) => folder.id === folderId) ? folderId : null
          } : project
        )
      }));
    },
    addList(projectId, listName) {
    ...
    insertList(projectId, list, index) {
      store.update((state) => ({
        ...state,
        projects: state.projects.map((project) => {
          if (project.id !== projectId) return project;
    ...
    addCard(projectId, listId, title) {
    ...
    deleteCardFromList(projectId, listId, cardId) {
      store.update((state) => ({
        ...state,
        projects: state.projects.map(
          (project) => project.id === projectId ? {
            ...project,
            lists: project.lists.map(
              (list) => list.id === listId ? { ...list, cards: list.cards.filter((card) => card.id !== cardId) } : list
            )
          } : project
        )
      }));
    },
    insertCard(projectId, listId, card, index) {
      store.update((state) => ({
        ...state,
        projects: state.projects.map(
          (project) => project.id === projectId ? {
    ...
    toggleCardDone(projectId, listId, cardId) {
      store.update((state) => ({
        ...state,
        projects: state.projects.map(
          (project) => project.id === projectId ? {
            ...
            lists: project.lists.map(
              (list) => list.id === listId ? {
                ...
                cards: list.cards.map(
                  (card) => card.id === cardId ? { ...card, done: !card.done } : card
                )
              } : list
            )
          } : project
    ...
    updateCard(projectId, listId, cardId, input) {
      store.update((state) => ({
        ...state,
        projects: state.projects.map(
          (project) => project.id === projectId ? {
              ...
              (list) => list.id === listId ? {
                ...
                cards: list.cards.map(
                  (card) => card.id === cardId ? {
                    ...
                    parentId: input.parentId !== void 0 ? input.parentId : card.parentId,
    ...
    moveCard(projectId, fromListId, toListId, cardId) {
    ...
    replaceListCards(projectId, listId, cards) {
      store.update((state) => ({
        ...state,
        projects: state.projects.map(
          (project) => project.id === projectId ? {
