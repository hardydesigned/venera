import {
  Icon_default
} from "./chunk-VO6FPRNL.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  snippet,
  spread_props,
  wrap_snippet
} from "./chunk-QKE3DCJT.js";
import {
  append,
  comment
} from "./chunk-34PGSFHB.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  noop,
  pop,
  push
} from "./chunk-B5ALGTIW.js";

// node_modules/.pnpm/@lucide+svelte@0.561.0_svelte@5.53.0/node_modules/@lucide/svelte/dist/icons/panel-left.svelte
Panel_left[FILENAME] = "node_modules/.pnpm/@lucide+svelte@0.561.0_svelte@5.53.0/node_modules/@lucide/svelte/dist/icons/panel-left.svelte";
function Panel_left($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, Panel_left);
  let props = rest_props($$props, ["$$slots", "$$events", "$$legacy"], "props");
  const iconNode = [
    [
      "rect",
      { "width": "18", "height": "18", "x": "3", "y": "3", "rx": "2" }
    ],
    ["path", { "d": "M9 3v18" }]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => (
      /**
      * @component @name PanelLeft
      * @description Lucide SVG icon component, renders SVG Element with children.
      *
      * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/panel-left
      * @see https://lucide.dev/guide/packages/lucide-svelte - Documentation
      *
      * @param {Object} props - Lucide icons props and any valid SVG attribute
      * @returns {FunctionalComponent} Svelte component
      *
      */
      Icon_default(node, spread_props({ name: "panel-left" }, () => props, {
        get iconNode() {
          return iconNode;
        },
        children: wrap_snippet(Panel_left, ($$anchor2, $$slotProps) => {
          var fragment_1 = comment();
          var node_1 = first_child(fragment_1);
          add_svelte_meta(() => snippet(node_1, () => $$props.children ?? noop), "render", Panel_left, 62, 2);
          append($$anchor2, fragment_1);
        }),
        $$slots: { default: true }
      }))
    ),
    "component",
    Panel_left,
    61,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  Panel_left = hmr(Panel_left);
  import.meta.hot.accept((module) => {
    Panel_left[HMR].update(module.default);
  });
}
var panel_left_default = Panel_left;

export {
  panel_left_default
};
//# sourceMappingURL=chunk-FTMQ5H4W.js.map
